...
def _parse_csv(value: str) -> list[str]:
    """Parse comma-delimited configuration values into a trimmed list."""
...
def _as_bool(value: str, default: bool = False) -> bool:
    """Convert env-style truthy/falsey strings into a boolean with fallback."""
...
def _load_settings() -> Settings:
    ...
    return Settings(
        ...
        cors_origins=_parse_csv(origins),
        ...
        allowed_hosts=_parse_csv(allowed_hosts) or ["127.0.0.1", "localhost", "testserver"],
        trust_proxy_headers=_as_bool(os.getenv("TRUST_PROXY_HEADERS", "false"), default=False),
...
def _require_metrics_key(
    ...
    """Enforce optional API key auth for metrics endpoint access."""

    if not settings.metrics_api_key:
        return
    if x_metrics_key != settings.metrics_api_key:
        raise HTTPException(status_code=401, detail="Unauthorized")
...
MetricsAuthDep = Annotated[None, Depends(_require_metrics_key)]
...
@app.get("/metrics")
def metrics(_: MetricsAuthDep = None) -> PlainTextResponse:
